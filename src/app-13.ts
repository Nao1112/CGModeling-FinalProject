...
class ThreeJSContainer {
    ...
    private createScene = () => {
        ...
        const bezier = (p0: THREE.Vector3, p1: THREE.Vector3,
            ...
            const result = p0.clone().multiplyScalar((1.0 - t) ** 3.0).add(p1.clone().multiplyScalar((3.0 * t * (1.0 - t) ** 2.0)).add(p2.clone().multiplyScalar((3.0 * t ** 2.0 * (1.0 - t)))).add(p3.clone().multiplyScalar((t ** 3.0))));
            return result;
        }
        //ベジエ曲線の微分
        const bezierBibun = (p0: THREE.Vector3, p1: THREE.Vector3, p2: THREE.Vector3, p3: THREE.Vector3, t: number): THREE.Vector3 => {
            const result = p1.clone().sub(p0).multiplyScalar(3.0 * (1.0 - t) ** 2.0).add(p2.clone().sub(p1).multiplyScalar(6.0 * (1.0 - t) * t)).add(p3.clone().sub(p2).multiplyScalar(3 * t ** 2.0));
            return result;
        }
        ...
        const update: FrameRequestCallback = (_time) => {
            ...
            //接線ベクトルの長さ
            const d = bezierBibun(points[0], points[1], points[2], points[3], t);
            const speed = d.length();

            //一定の速度にする
            const moveSpeed = 10.0;
            t += moveSpeed * delta / speed;
            if (t > 1.0) {
                t -= 1.0;
            }
